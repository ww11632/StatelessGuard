...
async function getReceiptAndTx(txHash) {
    ...
    try {
        ...
        const ethersMod = await Promise.resolve().then(() => __importStar(require('ethers'))).catch(() => null);
...
exports.verifyByTx = (0, https_1.onRequest)({ region: REGION, timeoutSeconds: TIMEOUT_SECONDS }, async (req, res) => {
    ...
    try {
        // eslint-disable-next-line @typescript-eslint/no-var-requires
        const arr = require('./demoTxMap.json');
