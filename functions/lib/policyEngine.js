...
function loadPolicy(policyName) {
    try {
        // 嘗試多個路徑（適配不同運行環境）
        const possiblePaths = [
            (0, path_1.join)(__dirname, '..', 'policies', `${policyName}.yaml`), // 開發環境（從 functions/ 目錄運行）
            (0, path_1.join)(process.cwd(), 'policies', `${policyName}.yaml`), // 從項目根目錄運行
            (0, path_1.join)(__dirname, 'policies', `${policyName}.yaml`), // 編譯後的 lib/ 目錄
        ];
        for (const yamlPath of possiblePaths) {
            try {
                const yamlContent = (0, fs_1.readFileSync)(yamlPath, 'utf-8');
                const policy = yaml.load(yamlContent);
                return policy;
            }
            catch (e) {
                // 繼續嘗試下一個路徑
        ...
        // 嘗試載入 JSON（後備方案）
        const jsonPaths = [
            (0, path_1.join)(__dirname, '..', 'policies', `${policyName}.json`),
            (0, path_1.join)(process.cwd(), 'policies', `${policyName}.json`),
            (0, path_1.join)(__dirname, 'policies', `${policyName}.json`),
        ];
        for (const jsonPath of jsonPaths) {
            try {
                const jsonContent = (0, fs_1.readFileSync)(jsonPath, 'utf-8');
                const policy = JSON.parse(jsonContent);
                return policy;
            }
            catch (e2) {
                // 繼續嘗試下一個路徑
...
function evaluateCondition(condition, context) {
    try {
        ...
        expr = expr.replace(/proof\.(\w+)/g, (match, field) => {
            const value = context.proof?.[field];
            if (value === undefined)
                return 'undefined';
        ...
        if (inMatch) {
            ...
            const value = context[varName] || context.proof?.[varName];
...
// 評估 policy
function evaluatePolicy(policyName, context) {
